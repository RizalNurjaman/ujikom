...
<hr>
    ...
    <tbody>
        ...
        $no=1;
        @endphp
        @foreach($golongin as $golongins)
        <tr>
            <td>{{$no++}}</td>
            <td>{{$golongins->kode_golongan}}</td>
            <td>{{$golongins->nama_golongan}}</td>
            <td>{{$golongins->besaran_uang}}</td>
            
        <td><a href="{{route('golongan.edit',$golongins->id)}}" class="btn btn-warning">Update</a></td> 
        </td>
        <th>
                                    {!!Form::open(['method'=>'DELETE','route'=>['golongan.destroy',$golongins->id]])!!}
