...
<br><br>
	...
	<tbody>
		...
		$no=1;
		@endphp
		@foreach($pegawaw as $pegawaws)
		<tr>
			<td>{{$no++}}</td>
			<td>{{$pegawaws->nip}}</td>
			<td>{{$pegawaws->User->name}}</td>
			<td>{{$pegawaws->User->email}}</td>
			<td>{{$pegawaws->User->permision}}</td>
			<td>{{$pegawaws->Mjabatan->nama_jabatan}}</td>
			<td>{{$pegawaws->Mgolongan->nama_golongan}}</td>
			
	   <th><img  width="" height="50px" class="img-circle" src="asset/image/{{$pegawaws->foto}}"></th>

		<td><a href="{{route('pegawai.edit',$pegawaws->id)}}"class="btn btn-warning">edit</a></td>	
		</td>

		<td>
		{!!Form::open(['method'=>'DELETE','route'=>['pegawai.destroy',$pegawaws->id]])!!}
