...
<center><h1>Data Jabatan</h1>
	...
	<tbody>
		...
		$no=1;
		@endphp
		@foreach($jabatin as $jabatins)
		<tr>
			<td>{{$no++}}</td>
			<td>{{$jabatins->kode_jabatan}}</td>
			<td>{{$jabatins->nama_jabatan}}</td>
			<?php $jabatins->besaran_uang=number_format($jabatins->besaran_uang,2,',','.') ?>
			<td>{{$jabatins->besaran_uang}}</td>
			
		<td><a href="{{route('jabatan.edit',$jabatins->id)}}" class="btn btn-warning">Update</a></td>	
		</td>
		<td>

		{!!Form::open(['method'=>'DELETE','route'=>['jabatan.destroy',$jabatins->id]])!!}
